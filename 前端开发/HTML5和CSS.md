...
补充：添加CSS样式 text-decoration：规定添加到文本的修饰，none表示定义标准的文本。可以用这种方式来去除超链接的下划线。
...
- radio可以添加name(表示控件分组，同组只能选中一个，即单选功能)，再加上checked表示默认选中。可以添加value属性，之后提交的值就是value的值。
- checkbox可以添加checked表示默认选中。可以添加value属性，之后提交的值就是value的值。
- file可以添加multiple实现文件多选功能

### 1.4.2 select标签

下拉菜单：select嵌套option，select是下拉菜单整体，option是下拉菜单的每一项(需要默认选中的一项只需写\<option selected\>)即可。select需要添加name属性，option需要添加value属性(表示提交的值)。
...
> 提示：按钮需配合 form 标签（表单区域）才能实现对应的功能。按钮无需添加name属性。
