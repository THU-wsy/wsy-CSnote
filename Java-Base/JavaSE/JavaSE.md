...
文档注释内容可以被JDK提供的工具 javadoc 所解析，生成一套以网页文件形式呈现的程序说明文档。操作命令如下：
...
逻辑运算符，操作的**都是boolean类型**的变量或常量，而且运算的结果也是boolean类型的值。
...
**解释：数组与可变参数是冲突的，所以子类的add方法是重写了父类的add方法，因此由多态性调用子类的add方法**
...
Object类的clone()方法：当某个对象调用这个方法时，会复制一个一模一样的新对象返回。
...
如果我们从Main的main()方法处运行程序，事实上JVM就调用了 `Main.main()`，于是会输出：
...
非匿名的局部内部类是定义在方法、代码块、构造器等执行体中的类，和局部变量一样，只在作用域中有效。一般在开发中较少使用。
...
**抽象枚举**：枚举类中也可以直接定义抽象方法，不过必须让每个枚举对象自己实现该方法
...
**枚举一般用于表示一组信息，然后作为参数进行传输**。如果选择定义一个一个的常量来表示一组信息，并作为参数传输，则参数值不受约束；但如果选择定义枚举表示一组信息，并作为参数传输，则代码可读性好，且参数值受到了约束。
...
注解（Annotation）是从`JDK5.0`开始引入的，以`@注解名`在代码中存在，它是代码中的特殊标记，作用是让程序根据注解信息来决定该怎么执行。一些注解是在**类编译**时进行解析，一些注解是在**运行**时进行解析。
...
`@Override`：用于检测被标记的方法为有效的重写方法，如果不是，则报编译错误。它只能标记在方法上。它会被编译器读取。

`@Deprecated`：用于表示被标记的数据已经过时。它会被编译器读取。

`@SuppressWarnings`：用于抑制编译警告，当我们不希望看到警告信息的时候，可以使用该注解来抑制警告信息。它会被编译器读取。
...
元注解：对现有的注解进行解释说明的注解。JDK5在java.lang.annotation包定义了4个标准的元注解。

#### 1、@Target

用于约束注解只能在哪些地方标注，例如`@Target(ElementType.TYPE)`。可以通过枚举类型ElementType的常量对象来指定：
...
- SOURCE：注解只存在于源码阶段，编译生成的字节码文件中不存在
- CLASS（默认值）：注解保留到字节码文件中，但在运行阶段不存在
- RUNTIME（开发常用）：注解一直保留到运行阶段，**注意只有RUNTIME生命周期的注解才能通过反射读取到**
...
- **对于注解中的抽象方法，如果没有default默认返回值，则使用时必须指定返回值**。格式是`方法名 = 返回值`，如果只有一个抽象方法需要赋值，且方法名为`value`，则可以省略`value=`，所以如果注解只有一个抽象方法成员，建议方法名命名为value。
...
自定义注解必须配上对应的解析处理流程才有意义。我们自定义的注解，**只能通过反射进行读取**，所以自定义注解的生命周期必须是`RetentionPolicy.RUNTIME`。

注解标注在哪个成分上，我们就必须先获取那个成分对象：

- 比如注解标注在成员方法上，则要先获得该成员方法对应的Method对象，再来获取上面的注解
- 比如注解标注在类上，则要先获得该类的Class对象，再来获取上面的注解
- 比如注解标注在成员变量上，则要先获得该成员变量对应的Field对象，再来获取上面的注解
...
- @Test标注的方法本身必须是public，非抽象的，非静态的，void无返回值，无参数的
...
以Integer为例，如果**调用的是`valueOf()`方法**，根据它的底层原理：
...
> 解释：`b += 10;`这里就等价于`b = b + 10;`此时右边的`b + 10`根据自动拆箱计算得到结果为10，然后再进行自动装箱，即等价于`b = Integer.valueOf(10);`所以此时b指向常量池中值为10的Integer对象。
...
**注意**：如上返回值为StringBuffer的API支持`链式调用`，因为其底层返回的就是`this`，例如：
...
**泛型的作用**：泛型提供了在**编译阶段**约束所能操作的数据类型，并自动进行检查的能力。这样可以避免强制类型转换，以及可能出现的异常。

**泛型的本质：把具体的数据类型作为参数传给类型变量**。

**注意**：泛型的类型变量，只能使用**引用数据类型**进行赋值（不能使用基本数据类型，但可以使用包装类替换）

**泛型擦除**：事实上，一旦程序被编译成class文件，**class文件中就不存在泛型的类型参数了**，这就是泛型擦除。因此，泛型类并没有自己独有的Class对象，比如并不存在`List<String>.class`，而只有`List.class`。
...
![image-20240609105206603](images/image-20240609105206603.png)
...
> **注意**：`Arrays.asList(T ... arr)`方法返回的List集合，并不是ArrayList的实例，而是一个**固定长度**的List集合，其长度无法被修改。
...
Collections工具类中提供了多个`synchronizedXxx()`方法，可以根据指定集合得到一个新的线程安全的集合：
...
**第6步**：如果第5步中所比较的两个hash值相同，则会进行源码中`(k = e.key) == key || key.equals(k)`的判断，也就是若equals()返回false，则(key1,value1)添加成功，如上调用addEntry()方法即可；否则，认为key1和key2是相同的，**此时直接将(key2,value2)对象中的value属性赋值为value1**。
...
LinkedHashMap是HashMap的子类，它在HashMap使用的数组+单向链表+红黑树的基础上，又增加了一对双向链表，记录添加的(key,value)的先后顺序。其底层内部定义的Entry对象继承了HashMap的Node对象，而且额外添加了**before和after指针**：
...
HashMap底层不是直接用key的hashCode值与`table.length-1`计算求下标的，而是先对key的hashCode值又进行了一个哈希运算`hash()`，这是为了保证**尽量均匀分布**。该哈希算法的思路是将hashCode值的**高位二进制值与低位二进制值进行了异或**，让高位二进制值也参与到index的计算中，这样做的原因是：

因为一个HashMap的table数组一般不会特别大（至少在不断扩容之前），那么`table.length-1`的大部分高位都是0，**直接用hashCode和table.length-1进行按位与运算的话，就会导致总是只有最低的几位是有效的**，那么就算你的`hashCode()`实现的再好也难以避免发生碰撞，这时让高位参与进来的意义就体现出来了。它对hashcode的低位添加了随机性并且混合了高位的部分特征，显著减少了哈希冲突的发生。

（2）key-value中的key是否可以修改？

key-value添加到HashMap后，**HashMap中会存储key的hash值**，这样就不用在每次查找时重新计算每一个Entry的hash值了，因此如果已经put到Map中的key-value，再修改key的属性，而这个属性又参与hash值的计算，那么会导致匹配不上。所以不建议修改key。

（3）JDK1.7中HashMap的循环链表是怎么回事？如何解决？

![](images/2023071966666.jpg)

HashMap死循环只会发生在JDK1.7版本中，主要原因：头插法+链表+多线程并发+扩容。而在JDK1.8中，HashMap改用尾插法，解决了链表死循环的问题。但是，我们在多线程环境下本就不推荐使用HashMap，而应该使用线程安全的ConcurrentHashMap。
...
有很多`GBxxx`字符集，主要针对中文字符（GB就是国标），最常用的是GBK字符集。对于ASCII字符它使用单字节存储，而其他字符则采用双字节存储。
...
> 注1：对于不想进行序列化的属性，可以使用`transient`关键字修饰，它能阻止该属性序列化。注意，当进行反序列化时，被`transient`修饰的属性会被赋值为默认值。
> 
...
Java的BIO模型与UNIX的BIO模型相同。

### 13.2 NIO

Java的NIO模型实际上与UNIX的IO多路复用模型相似，它提供了非阻塞、面向缓冲、基于通道的IO，可以使用少量的线程来处理多个连接。`java.nio`包具有以下三个核心组件：

- **Buffer（缓冲区）**：NIO读写数据都是通过缓冲区进行操作的。读操作的时候将Channel中的数据填充到Buffer中，而写操作时将Buffer中的数据写入到Channel中。
- **Channel（通道）**：Channel是一个双向的、可读可写的数据传输通道，NIO通过Channel来实现数据的输入输出。通道是一个抽象的概念，它可以代表文件、套接字或者其他数据源之间的连接。
- **Selector（选择器）**：允许一个线程处理多个Channel，基于事件驱动的I/O多路复用模型。所有的Channel都可以注册到Selector上，由Selector来分配线程来处理事件。

![image-20240608221417051](images/image-20240608221417051.png)

Java的NIO中也用到了**零拷贝**技术来提升IO性能。零拷贝是指计算机执行IO操作时，CPU不需要将数据从一个存储区域复制到另一个存储区域，从而可以减少上下文切换以及CPU的拷贝时间。NIO中使用零拷贝的地方：

- `MappedByteBuffer`是NIO基于内存映射（`mmap`）这种零拷贝方式提供的一种实现，底层实际是调用了Linux内核的`mmap`系统调用。它可以将一个文件或者文件的一部分映射到内存中，形成一个虚拟内存文件，这样就可以直接操作内存中的数据，而不需要通过系统调用来读写文件。
- `FileChannel`的`transferTo()/transferFrom()`是NIO基于发送文件（`sendfile`）这种零拷贝方式提供的一种实现，底层实际是调用了Linux内核的`sendfile`系统调用。它可以直接将文件数据从磁盘发送到网络，而不需要经过用户空间的缓冲区。

### 13.3 AIO

Java的AIO模型与UNIX的AIO模型相同。
...
在JVM加载类之后，方法区中就产生了一个Class类型的对象，也称之为`字节码对象`（一个类对应一个Class对象），这个对象就包含了完整的类的结构信息。这个对象就像一面镜子，透过这个镜子可以看到类的结构，所以我们形象地称之为**反射**。

简单来说，**反射技术，指的是加载类的字节码到内存，并以编程的方式解刨出类中的各个成分（成员变量、方法、构造器等）**。
...
```java
Class clazz = ClassLoader.getSystemClassLoader().loadClass("com.thuwsy.Person");
System.out.println(clazz); // class com.thuwsy.Person
```
...
3. 可以为被代理对象的所有方法做代理。
4. 非常的灵活，支持为**任意接口类型的实现类对象**做代理，也可以直接**为接口本身**做代理。
...
Lambda表达式是一个语法糖，其作用是：**简化匿名内部类代码的书写**。本质上Lambda表达式就是一个接口的匿名实现类的匿名对象。

> 语法糖指的是编程语言为了方便程序员开发程序而设计的一种特殊语法。实现相同的功能，基于语法糖写出来的代码往往更简洁易读。
...
**（3）规则三**：如果方法体只有一行代码，可以省略大括号不写，同时要省略分号；此时，如果这行代码是return语句，就必须去掉return。
...
3. 流是**惰性的**，只有在启动终止操作时才会对源数据进行计算，而且只在需要时才会消耗源元素。
...
多个中间操作可以连接起来形成一个流水线，除非流水线上触发终止操作，否则中间操作不会执行任何的处理。所以在终止操作时一次性全部进行处理，称为**惰性求值**。

**注意**：对于串行流，默认会将一个元素依次执行完所有流中的操作过程，然后再处理下一个元素(如果是像`sorted()`这种必须用到所有元素的操作则除外)。
...
收集Stream流，就是把Stream流操作后的结果转回到集合或者数组返回。
