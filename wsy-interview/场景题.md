...
其中前三个步骤无前后因果关系，可以并行执行，而最后一个对账的步骤则需要等待前三个步骤完成之后再执行。除此之外，不同用户的数据迁移也可以用多线程并行执行。示例代码如下：

```java
public class MoveUserService {
    /**
     * 线程池
     * 核心线程数3、最大线程数15、阻塞队列长度为200
     */
    private static final ThreadPoolExecutor threadPool = new ThreadPoolExecutor(
            3, 15, 60, TimeUnit.SECONDS,
    ...
    public static void main(String[] args) {
        MoveUserService moveUserService = new MoveUserService();
        // 假设有5个用户进行数据迁移
        List<Long> userIdList = List.of(101L, 102L, 103L, 104L, 105L);
        moveUserService.moveUserData(userIdList);
    }

    /**
     * 迁移用户数据的主流程，多个用户间通过多线程并行执行迁移逻辑
     */
    public void moveUserData(List<Long> userIdList) {
        for (Long userId : userIdList) {
            threadPool.execute(() -> doMoveUserData(userId));
        }
    }
     ...
     * 3. 迁移同步用户的额外扩展信息；
     * 4. 执行对账，确保迁移数据的一致性。
     * 设置计数器CountDownLatch的值为3，从而并行执行前三个步骤
     */
    private void doMoveUserData(Long userId) {
        CountDownLatch countDownLatch = new CountDownLatch(3);
        syncUserBasicInfo(userId, countDownLatch);
        syncUserAccountInfo(userId, countDownLatch);
        syncUserExtendInfo(userId, countDownLatch);
        checkUserInfo(userId, countDownLatch);
    }

    private void syncUserBasicInfo(Long userId, CountDownLatch countDownLatch) {
        threadPool.execute(() -> {
            // 迁移同步用户的基本信息......
    ...
    private void syncUserAccountInfo(Long userId, CountDownLatch countDownLatch) {
        threadPool.execute(() -> {
            // 迁移同步用户的账户信息......
            countDownLatch.countDown();
        });
    ...
    private void syncUserExtendInfo(Long userId, CountDownLatch countDownLatch) {
        threadPool.execute(() -> {
            // 迁移同步用户的额外扩展信息......
            countDownLatch.countDown();
        });
    ...
    private void checkUserInfo(Long userId, CountDownLatch countDownLatch) {
        try {
            countDownLatch.await();
            System.out.println("check success, userId = " + userId);
...
执行上述代码后，我们会发现一行输出都没有打印，实际上是产生了线程池死锁。原因如下：

1. 该场景涉及主子任务，主任务是对每个不同用户开启多线程，子任务是具体执行各种数据同步逻辑。这两种**不同类型的主子任务共用了一个线程池**。
2. 由于线程池的核心线程数只有3，而用户数量有5个，所以会有3个主任务占用核心线程、剩下2个主任务位于等待队列。
3. 而这3个占用核心线程的主任务中，又各自拆分了3个子任务（共9个子任务），且因为主子任务共用了一个线程池，所以这9个子任务都位于等待队列。
4. 因此，3个占用核心线程的主任务一直在等待子任务执行完毕，而子任务一直位于等待队列中无法执行，就导致了线程池死锁。

### 2.3 解决方案

在涉及主子任务的场景中，只要**主任务和子任务不共用同一个线程池**，就能避免上述死锁问题的产生。除此之外，我们还建议给异步任务的执行设置超时时间，作为一种兜底策略，例如在上述代码中改为`countDownLatch.await(5, TimeUnit.SECONDS)`，当发生超时时抛出异常告警、进行人工介入，而不至于导致整个应用的卡死。
...
## 1. 大流量的基础服务挂了后重启需要考虑的因素

假设有一个基础服务流量很大，线上有100台机器，如果该服务某次发布出现错误导致挂了，那么重启需要考虑很多因素。

首先我们不能走正常发布的流程，因为正常发布都是分批发布，例如每批次发布20台机器，那么在正常情况下线上始终能保证80台机器可用。但是如果整个服务直接挂掉了，再走正常的分批发布流程，第一批次只重启20台机器，未必能抗住线上的大流量。

所以应该尽量将整个集群（100台）一起启动，而不是分批发布。除此之外，这个基础服务出错的时候可能也会导致缓存的一并丢失，所以在重启时还要做一些缓存的预热，因此像这种高并发系统在发布的时候一定要写好预案。
