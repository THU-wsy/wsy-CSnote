...
2. **全连接层**的 BatchNorm（nn.BatchNorm1d）：输入形状 (batch, features)，会对每个特征维度，在整个 batch 上求均值/方差，所以可学习的参数 $$\gamma、\beta$$ 总计 `2 * features` 个
3. **卷积层**的 BatchNorm（nn.BatchNorm2d）：输入形状 (batch, channels, H, W)，会对每个通道，在整个 batch × H × W 上求均值/方差，所以可学习的参数 $$\gamma、\beta$$ 总计 `2 * channels` 个
4. BatchNorm 通常会**在全连接层/卷积层之后、激活函数之前**使用
...
#### 核心思想
**残差连接**（Skip Connection）：$\mathbf{y} = F(\mathbf{x}) + \mathbf{x}$

解决深层网络**退化**问题（网络越深，训练误差反而不降）。
...
- 1×1 降维 → 3×3 卷积 → 1×1 升维，减少计算量

### 3.8 稠密连接网络（DenseNet）

#### 核心思想
每个层的输出作为**所有后续层的额外输入**（密集连接）：
...
## 六、学习率、batch_size 与 epochs 参考

| 参数 | 建议值 | 说明 |
|------|--------|------|
| lr | 0.1 ~ 0.001 | SGD 从大开始；Adam 用 0.001 |
| batch_size | 32/64/128 | 受 GPU 显存限制 |
| num_epochs | 10 ~ 100+ | 看收敛情况 |
| weight_decay | 1e-4 ~ 1e-3 | L2 正则化 |
| momentum | 0.9 | SGD 常用动量 |
...
2. **Batch Normalization**：加速训练，允许大学习率，有一定正则化效果
